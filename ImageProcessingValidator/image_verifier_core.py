...
class ImageVerifier:
    ...
    def __init__(self, source_folder: str, target_folder: str, missing_folder: str, max_workers: int = None):
        ...
        if not os.path.exists(missing_folder):
            ...
            self._log(f"创建输出文件夹: {missing_folder}")
        else:
            self._log(f"输出文件夹已存在: {missing_folder}")
    ...
    def scan_source_images(self, source_extensions: Tuple[str, ...] = ('.jpg', '.jpeg', '.png')) -> List[
        ...
        self._log(f"找到 {len(self.source_images)} 张原始图片. 耗时: {elapsed_time:.2f}秒")
    ...
    def verify_completeness(self, expected_suffixes: List[str]) -> Tuple[List[str], List[Tuple[str, Set[str]]]]:
        ...
        elapsed_time = time.time() - start_time
        self._log(f"完成处理完整性验证. 耗时: {elapsed_time:.2f}秒")
    ...
    def _copy_missing_files(self, missing_queue: Queue):
        ...
        self._log(f"\n开始复制 {len(copy_tasks)} 个缺失或不完整的文件...")
        ...
        def progress_reporter():
            ...
            while not all(future.done() for future in futures):
                ...
                if current_percent != last_percent:
                    self._log(f"\r{task_name}进度: {current_percent}% ({self.processed_files}/{self.total_files})")
                    last_percent = current_percent
                time.sleep(0.5)

            self._log(f"\r{task_name}进度: 100% ({self.total_files}/{self.total_files})")
    ...
    def print_summary(self, expected_count: int, naming_format: str):
        ...
        if self.missing_images:
            ...
            if len(self.missing_images) > 10:
                self._log(f"  ... 以及 {len(self.missing_images) - 10} 个更多 ...")
            self._log(f"  共 {len(self.missing_images)} 张图片未处理，已复制到 '{self.missing_folder}'")
        else:
            self._log("  所有原始图片都已进行处理，没有完全遗漏的图片。")

        # 2. 显示处理不完整的图片
        self._log("\n2. 处理不完整的图片:")
        if self.incomplete_images:
            for img, missing_suffixes in self.incomplete_images[:10]:  # 只显示前10个
                self._log(f"  - {img} (缺少版本: {', '.join(sorted(missing_suffixes))})")
            if len(self.incomplete_images) > 10:
                self._log(f"  ... 以及 {len(self.incomplete_images) - 10} 个更多 ...")
            self._log(f"  共 {len(self.incomplete_images)} 张图片处理不完整，已复制到 '{self.missing_folder}'")
        else:
            self._log(f"  所有已处理的图片都有完整的{expected_count}个版本，没有处理不完整的图片。")
        ...
        if self.naming_errors:
            ...
            if len(self.naming_errors) > 10:
                self._log(f"  ... 以及 {len(self.naming_errors) - 10} 个更多 ...")
            self._log(f"  共 {len(self.naming_errors)} 张图片命名不规范")
        else:
            self._log(f"  所有处理后的图片命名均符合规范: {naming_format}")
...
class NamingPattern:
    ...
    def create_numeric_suffix_pattern(
        ...
        参数:
        delimiter: 文件名与后缀之间的分隔符
        extension: 文件扩展名
        min_digits: 最小数字位数
        max_digits: 最大数字位数，None表示不限制
    ...
    def create_range_suffix_pattern(
        ...
        delimiter: 文件名与后缀之间的分隔符
        extension: 文件扩展名
        suffix_range: 后缀数字范围，如(1, 9)表示1到9
